...
class Quan_Ly_Du_Lieu:
    ...
    def khoi_tao_du_lieu(self):
        with sqlite3.connect(self.database) as ket_noi: # Kết nối với database đặt bí danh là ket_noi dùng with tự động đóng kết nối, kể cả khi có lỗi
    ...
    def tat_ca_don(self):
        with sqlite3.connect(self.database) as ket_noi:
            con_tro = ket_noi.cursor()
            con_tro.execute("SELECT * FROM don_hang")
            danh_sach = con_tro.fetchall() # lấy ra tất cả dòng kết quả từ câu lệnh truy vấn sql
            return danh_sach # Trả về danh sách để hiển thị,hàm chỉ truy vấn SELECT (đọc dữ liệu), không làm thay đổi database nên không cần commit()
    ...
    # Phương thức kiểm tra tồn tại,hàm chỉ truy vấn SELECT (đọc dữ liệu), không làm thay đổi database nên không cần commit()
    ...
    # Phương thức tìm kiếm đơn hàng theo ký tự nhập vào, hàm chỉ truy vấn SELECT (đọc dữ liệu), không làm thay đổi database nên không cần commit()
    ...
    def lay_trang_thai(self, ma_don_hang):
        with sqlite3.connect(self.database) as ket_noi:
            ...
            if ket_qua is not None:
                ...
                return None # trả về none nếu không tìm thấy, hàm chỉ truy vấn SELECT (đọc dữ liệu), không làm thay đổi database nên không cần commit()
...
class Quan_Ly_Don_Hang:
    ...
    # Phương thức thêm đơn hàng với vào database
    ...
    def tim_kiem_don_hang(self):
        ...
        if tu_khoa=="":
            ...
            return
        # gọi phương thức tìm kiếm từ database
