...
class Cua_So_Dang_Nhap:
    ...
    def xu_ly_dang_nhap(self):
        tai_khoan = self.nhap_tai_khoan.get().strip().replace(" ", "").upper()
        mat_khau = self.nhap_mat_khau.get().strip().replace(" ", "")
...
class Quan_Ly_Don_Hang:
    ...
    def __init__(self):
        ...
        self.file_bao_cao = os.path.join(thu_muc_goc, "Báo cáo đơn hàng.xlsx")
    ...
    def them_don_hang(self):
        ma_don_hang = self.nhap_ma_don_hang.get().strip().replace(" ", "").upper() # xóa khoảng trắng, viết hoa toàn bộ ký tự
        ma_san_pham = self.nhap_ma_san_pham.get().strip().replace(" ", "").upper()
        ten_san_pham = " ".join(self.nhap_ten_san_pham.get().split()).title() # viết hoa ký từ đầu mỗi từ
        khach_hang = " ".join(self.nhap_khach_hang.get().split()).upper()
        don_vi_tinh = " ".join(self.nhap_don_vi_tinh.get().split()).title()
        so_luong = self.nhap_so_luong.get().strip().replace(" ", "")
        don_gia = self.nhap_don_gia.get().strip().replace(" ", "")
    ...
    def sua_don_hang(self):
        ma_don_hang = self.nhap_ma_don_hang.get().strip().replace(" ", "").upper() # xóa khoảng trắng, viết hoa toàn bộ ký tự
        ma_san_pham = self.nhap_ma_san_pham.get().strip().replace(" ", "").upper()
        ten_san_pham = " ".join(self.nhap_ten_san_pham.get().split()).title()
        khach_hang = " ".join(self.nhap_khach_hang.get().split()).upper()
        don_vi_tinh = " ".join(self.nhap_don_vi_tinh.get().split()).title()
        so_luong = self.nhap_so_luong.get().strip().replace(" ", "")
        don_gia = self.nhap_don_gia.get().strip().replace(" ", "")
    ...
    def xoa_don_hang(self):
        tu_khoa = self.nhap_ma_don_hang.get().strip().replace(" ", "").upper()
        if tu_khoa == "":
