...
class Quan_Ly_Don_Hang:
    ...
    def tao_giao_dien(self):
        ...
        # Tạo menu chức năng thao tác với đơn hàng
        menu_chuc_nang = tk.Frame(self.giao_dien)
        menu_chuc_nang.pack(pady=5)

        tk.Label(menu_chuc_nang, text="Mã đơn hàng:", font=("Arial", 10)).pack(side="left", padx=(0, 5))
        self.nhap_ma_don_hang = ttk.Entry(menu_chuc_nang, width=12, style="nhapdulieu.TEntry")
        self.nhap_ma_don_hang.pack(side="left", padx=(5))
        
        self.nut_tim_kiem = tk.Button(menu_chuc_nang, text="🔍 Tìm kiếm", width=13, bg="#d5e1f9", fg="#000000", font=("Arial",9, "bold"), relief="raised", bd=2)# relief="raised" tạo hiệu ứng nút, bd=2 độ dày viền nút 2px
        self.nut_tim_kiem.pack(side="left",padx=5)

        self.nut_hoan_thanh = tk.Button(menu_chuc_nang, text="✅ Hoàn thành đơn", width=18, bg="#c2fbd7", fg="#000000", font=("Arial", 9, "bold"), relief="raised", bd=2)
        self.nut_hoan_thanh.pack(side="left", padx=(5))

        self.nut_hien_thi = tk.Button(menu_chuc_nang, text="📋 Tất cả đơn", width=18, bg="#d5e1f9", fg="#000000", font=("Arial", 9, "bold"), relief="raised", bd=2)
        self.nut_hien_thi.pack(side="left", padx=5)

        self.nut_xoa_don = tk.Button(menu_chuc_nang, text="🗑️ Xóa đơn", width=13, bg="#EF4444", fg="#F3F0F0", font=("Arial", 9, "bold"), relief="raised", bd=2)
        self.nut_xoa_don.pack(side="left", padx=(5, 0))

        # Tạo menu làm việc với file dữ liệu
        menu_tien_ich = tk.Frame(self.giao_dien)
        menu_tien_ich.pack(pady=5)
        self.nut_them_don = tk.Button(menu_tien_ich, text="➕Thêm vào đơn", width=20, bg="#9ff0d9", fg="#000000", font=("Arial", 9, "bold"), relief="raised", bd=2)
        self.nut_them_don.pack(side="left", padx=(0, 10))

        self.nut_xuat_bao_cao = tk.Button(menu_tien_ich, text="⬇️ Xuất báo cáo", width=20, bg="#9ff0d9", fg="#000000", font=("Arial", 9, "bold"), relief="raised", bd=2)
    ...
    def dang_nhap_thanh_cong(self):
        self.cap_nhat_du_lieu() # Tai du lieu len bang hien thi
